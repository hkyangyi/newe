...
func (a *SysDepart) Add() error {
	...
	if len(a.Pid) == 0 {
		...
		var fdb SysDepart
		db.Db.Model(&SysDepart{}).Where("id = ?", a.Pid).First(&fdb)
		code := a.GetCode()
...
func (a *SysDepart) Edit() error {
	...
	err := db.Db.Transaction(func(tx *gorm.DB) error {
		...
		if odb.Pid != a.Pid {
			...
			a.Code = fdb.Code + "-" + code
			var items []SysDepart
			db.Db.Model(&SysDepart{}).Where("code like ?", odb.Code+"%").Or("org_code like ?", odb.Code+"%").Find(&items)
			for _, v := range items {
...
func (a *SysDepart) Del() error {
	var count int64
	db.Db.Table("sys_depart").Where("pid = ?", a.ID).Count(&count)
	if count > 0 {
		return errors.New("请先删除子分类")
	}
	db.Db.Table("sys_member").Where("depart_id = ?", a.ID).Count(&count)
	if count > 0 {
...
func (a *SysDepart) GetList(usdata SysMember) []SysDepart {
	...
	if usdata.Username == "admin" {
		db.Db.Table("sys_depart").Order("sort_no asc").Find(&items)
		list = SysDepartDigui(items, "", list)
	} else {
		var pdb SysDepart
		db.Db.Table("sys_depart").Where("code", usdata.OrgCode).Order("sort_no asc").First(&pdb)
		db.Db.Table("sys_depart").Where("code like ?", pdb.Code+"%").Order("sort_no asc").Find(&items)
...
func GetCodeByDepartId(departId string) string {
	var data SysDepartDict
	db.Db.Model(&SysDepartDict{}).Where("depart_id = ?", departId).First(&data)
	return data.ServeCode + strconv.Itoa(int(data.Code))
}
...
func DepartRulesByDeaprtId(id string) []SysDepartRules {
	var items []SysDepartRules
	db.Db.Table("sys_depart_rules").Where("depart_id  =  ?", id).Find(&items)
	return items
}
