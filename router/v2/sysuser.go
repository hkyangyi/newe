...
func ChangePassword(c *gin.Context) {
	...
	// 获取当前用户
	merdata, ok := a.C.Get("AdminAuthData")
	if !ok {
		a.LoginError(errors.New("登陆超时"))
		return
	}
	...
	if err := model.MemberChangePassword(mer.Merdb.ID, req.OldPassword, req.NewPassword); err != nil {
		a.Error(err)
		return
	}
	a.SUCCESS(nil)
...
func UploadAvatar(c *gin.Context) {
	...
	if err := db.Db.Model(&model.SysMember{}).Where("id = ?", mer.Merdb.ID).Update("headimgurl", url).Error; err != nil {
		a.Error(err)
		return
	}
