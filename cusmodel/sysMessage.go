...
type CustomerMessage struct {
	...
	Cid       string `gorm:"index:idx_user_created;size:64" json:"cid" form:"cid"` // 商户ID（与登录uuid一致）
...
func (CustomerMessage) TableName() string { return "sys_message" }

// 执行注册数据库
func regCustomerMessageTable() error {
	if db.Db == nil {
		return errors.New("db not initialized")
	}
	return db.Db.AutoMigrate(&CustomerMessage{})
}

func (a *CustomerMessage) Add() error {
	err := db.Db.Create(a).Error
	return err
}
func (a *CustomerMessage) Edit() error {
	if len(a.ID) != 32 {
...
func (a *CustomerMessage) ReadEnd() error {
	if len(a.ID) != 32 {
		return errors.New("缺少参数ID")
	}
...
func (a *CustomerMessage) Del() error {
	err := db.Db.Model(a).Delete(a).Error
	return err
}

func (a *CustomerMessage) GetList(where string, v ...interface{}) ([]CustomerMessage, error) {
	var list []CustomerMessage
	err := db.Db.Where(where, v...).Order("created_at desc").Find(&list).Error
	return list, err
}

func (a *CustomerMessage) GetPage(page utils.PageList, where string, v ...interface{}) (utils.PageList, error) {
	var list []CustomerMessage
	err := db.Db.Model(&CustomerMessage{}).Where(where, v...).Order("created_at desc").Count(&page.Total).Offset(page.GetOffice()).Limit(page.PageSize).Find(&list).Error
	page.List = list
	return page, err
}

func (a *CustomerMessage) GetOne(where string, v ...interface{}) (CustomerMessage, error) {
	var one CustomerMessage
	err := db.Db.Where(where, v...).First(&one).Error
	return one, err
}

func (a *CustomerMessage) SetRead() error {
