...
type CustomerOperationLog struct {
	ID           int64     `gorm:"primaryKey;autoIncrement" json:"id" form:"id"`                    // 日志主键
	Cid          string    `gorm:"type:varchar(32);index" json:"cid"`                               // 客户ID
...
func (CustomerOperationLog) TableName() string { return "customer_operation_log" }

func (a *CustomerOperationLog) Add() error {
	err := db.Db.Create(a).Error
	return err
}

// EnsureApiAndGetID 确保当前日志对应的接口在 sys_api_list 中存在，并返回接口ID
func (a *CustomerOperationLog) EnsureApiAndGetID(name string) (int64, error) {
	var api CustomerApiList
...
func (a *CustomerOperationLog) GetPage(page utils.PageList, where string, v ...interface{}) utils.PageList {
	var items []CustomerOperationLog
	db.Db.Model(&CustomerOperationLog{}).Where(where, v...).Count(&page.Total).Order("created_at desc").Offset(page.GetOffice()).Limit(page.PageSize).Find(&items)
	page.List = items
	return page
}
